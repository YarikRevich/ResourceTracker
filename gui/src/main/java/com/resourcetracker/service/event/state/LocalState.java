...
/**
 * Represents local state management model, which is used
 * to handle application state changes and exposes them for
 * the further usage.
 */
@Component
public class LocalState {
    @Getter
    @Setter
    static private boolean connectionEstablished;

    /**
     * Handles changes of connection establishment status.
     * @param event connection status event, which contains
