...
public class ConnectionStatusImageView
    implements IElementActualizable, IElementResizable, IElement<SplitPane> {
          ...
          if (LocalState.getConnectionEstablished()) {
            ...
            button.setGraphic(
                ConnectionStatusImageCollection.getSuccessfulConnectionStatusImage(
                    WindowHelper.getCircularElementSize(
                        LocalState.getWindowWidth(), properties.getStatusImageScale())));
          } else {
  ...
  public void handlePrefWidth() {
    if (Objects.isNull(((Button) getContent().getItems().get(0)).getGraphic())) {
      ((Button) getContent().getItems().get(0))
          .setGraphic(
              ConnectionStatusImageCollection.getFailedConnectionStatusImage(
                  WindowHelper.getCircularElementSize(
                      LocalState.getWindowWidth(), properties.getStatusImageScale())));
